    ...
    W: Write,
{
...

    {
        ...
        let mut base_path_str = base_path.to_string_lossy().to_string();
        ...
        let mut append = |path: &Path| {
            ...
            let _cannonical = canonical.to_string_lossy().to_string();
            let relativized = _cannonical.trim_start_matches(&base_path_str[..]);
            if path.is_dir() {
